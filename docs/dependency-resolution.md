...
1. **Verified against a real SDK** (`--ios-sdk-path <path>`, or
   ...
   only works on a real macOS toolchain). SDKs mirror the runtime's
   absolute framework/dylib paths under their own root
   (`<sdk>/System/Library/Frameworks/Foundation.framework/Foundation`), so
   this is a direct, authoritative existence check, not a guess.
   `.github/workflows/build.yml` runs this on every push against
   `test-payload/PatchedMacOSPayload.dylib` (which links
   `/usr/lib/libSystem.B.dylib` after the `-lSystem` fix -- see
   `test-payload/build-and-patch.sh`), so the tool itself is proven
   against a real SDK continuously, not just this repo's own offline
   tests.
